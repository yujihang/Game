...
(function($, window, document) {
	...
	var AI = {
		...
		moveChess: function(chessColor) {
			...
			if((maxWeight >= 10000 && maxWeight < 100000) || (maxWeight >= 250000 && maxWeight < 500000)) {
				hasWin(chessColor);
			} else {
				nextPlayer();
			}
		...
		weightStatus: function(nums, side1, side2, isAI) {
			...
			switch(nums) {
				case 1:
					if(side1 && side2) {
						weight = isAI ? 10 : 15; //独一,独立的一个子，两边没子
					}
					break;
				case 2:
					if(side1 && side2) {
						weight = isAI ? 50 : 100; //独二
					} else if(side1 || side2) {
						weight = isAI ? 5 : 10; //单二
					}
					break;
				case 3:
					if(side1 && side2) {
						weight = isAI ? 200 : 500; //独三
					} else if(side1 || side2) {
						weight = isAI ? 20 : 30; //单三
					}
					break;
				case 4:
					if(side1 && side2) {
						weight = isAI ? 2000 : 5000; //独四
					} else if(side1 || side2) {
						weight = isAI ? 100 : 400; //单四
					}
					break;
				case 5:
					weight = isAI ? 10000 : 100000; //五
					break;
				default:
					weight = isAI ? 250000 : 500000;
					break;
			}
			return weight;
		}
